...
RUN apk add --no-cache bzip2 ca-certificates coreutils libstdc++ tini w3m &&\
  ...
  wget http://dl.4players.de/ts/releases/${TS_SERVER_VER}/teamspeak3-server_linux_alpine-${TS_SERVER_VER}.tar.bz2 -O /tmp/teamspeak.tar.bz2 &&\
  mkdir /opt &&\
...
RUN adduser -u 4294967294 -g 4294967294 -h ${TS_DIRECTORY} -D teamspeak &&\
  mkdir /data &&\
